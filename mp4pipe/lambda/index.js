const util = require('./utils'),
	childProcess = require('./spawn-child'),
	path = require('path'),
	os = require('os'),
...
exports.handler = function (eventObject, context) {
	...
	return util.downloadFileFromS3(inputBucket, key, inputFile)
		.then(() => childProcess.spawn(
			'/opt/bin/ffmpeg',
			['-loglevel', 'error', '-y', '-i', inputFile, '-vf', `thumbnail,scale=${THUMB_WIDTH}:-1`, '-frames:v', '1', outputFile],
			{env: process.env, cwd: workdir}
		))
		.then(() => util.uploadFileToS3(OUTPUT_BUCKET, resultKey, outputFile, MIME_TYPE));
};
